...
# Check if the Docker image 'iwcd-ft-test-client-t:alpine' exists
if ! docker image inspect iwcd-ft-test-client-t:alpine >/dev/null 2>&1; then
    echo "Image 'iwcd-ft-test-client-t:alpine' not found. Building it first..."
    current_dir=$(pwd)
    cd ../../../t/alpine/ft-test-client || exit 1
    sh build-local.sh
    if [ $? -ne 0 ]; then
