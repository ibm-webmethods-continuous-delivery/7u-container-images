...
# Check if the Docker image 'iwcd-ft-test-client-s:alpine' exists
if ! docker image inspect iwcd-ft-test-client-s:alpine >/dev/null 2>&1; then
    echo "Image 'iwcd-ft-test-client-s:alpine' not found. Building it first..."
    current_dir=$(pwd)
    cd ../../../s/alpine/ft-test-client || exit 1
    sh build-local.sh
    if [ $? -ne 0 ]; then
